...
namespace MadMaps.Terrains
{
...
#endif
    {
        ...
        {
            get => UnityEditor.EditorPrefs.GetBool("TerrainWrapper_ComputeShaders", true);
        ...
        private void FinaliseObjects()
        {
            ...
            foreach (var keyValuePair in CompoundTerrainData.Objects)
            {
                ...
                if (!string.IsNullOrEmpty(prefabObjectData.ContainerMetadata))
                {
                    ...
                    if (!parent)
                    {
                        var currentTraversal = ObjectContainer.transform;
        ...
        private void FinaliseSplats()
        {
            ...
            // Splats
#if UNITY_2018_3_OR_NEWER
            var compoundLayers = CompoundTerrainData.TerrainLayerSplatData;
            var wrapperLayers = TerrainLayerSplatPrototypes;
            foreach (var compoundSplat in compoundLayers)
            {
                if (!wrapperLayers.Contains(compoundSplat.Key))
                {
            ...
            var sw = new Stopwatch();
            sw.Start();
            if (compoundLayers.Count > 0 && wrapperLayers.Count > 0)
            {
                var aRes = compoundLayers.GetValues().First().Width;
                if (ComputeShaders && BlendMMTerrainLayerUtility.ShouldCompute())
                {
                    const int maxChunkSize = 2048 * 2048 * 4;
                    var subdivisions = Mathf.CeilToInt((aRes * aRes * wrapperLayers.Count) / (float)maxChunkSize);
                    var subRes = aRes / subdivisions;

                    var splats = new float[subRes, subRes, wrapperLayers.Count];
                    ...
                    var dataBuffer = new ComputeBuffer(subRes * subRes * wrapperLayers.Count, sizeof(float));
                    ...
                    for (var u = 0; u < subdivisions; u++)
                    {
                        for (var v = 0; v < subdivisions; v++)
                        {
                            foreach (var compoundSplat in compoundLayers)
                            {
                                var index = wrapperLayers.IndexOf(compoundSplat.Key);
                                if (index < 0)
                                {
                                ...
                                combineShader.SetVector("_WriteDataSize",
                                    new Vector4(subRes, subRes, wrapperLayers.Count));

                                combineShader.SetBuffer(combineShaderKernel, "_ReadData", splatLayerData);
                                combineShader.SetVector("_ReadDataSize", new Vector4(aRes, aRes, wrapperLayers.Count));
                ...
                {
                    var splats = new float[aRes, aRes, wrapperLayers.Count];
                    foreach (var compoundSplat in compoundLayers)
                    {
                        var index = wrapperLayers.IndexOf(compoundSplat.Key);
                        if (index < 0)
            ...
            bool includeTerminatingLayer)
        {
            ...
            for (var i = Layers.Count - 1; i >= 0; i--)
            {
                ...
                if (wrappers != null)
                {
                    for (var j = 0; j < wrappers.Count; j++)
                    {
        ...
        public List<MadMapsTreeInstance> GetCompoundTrees(LayerBase terminatingLayer, bool includeTerminatingLayer = false, Bounds? bounds = null)
        {
            ...
            for (var i = Layers.Count - 1; i >= 0; i--)
            {
                ...
                var removals = layer.GetTreeRemovals();
                if (removals != null)
                {
        ...
        public void CreateCompoundCache(LayerBase layer)
        {
            ...
            var result = new CompoundMMTerrainLayer();
        ...
        {
            var layerIndex = GetLayerIndex(layer) - 1;
