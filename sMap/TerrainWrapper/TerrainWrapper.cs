...
namespace sMap.Terrains
{
    ...
    public class TerrainWrapper : MonoBehaviour, ILevelPreBuildStepCallback
    {
        ...
        {
            if (!WriteObjects || CompoundTerrainData.Objects.Count == 0)
            {
                /*if (ObjectContainer)
                {
...
#if UNITY_EDITOR
            if (UnityEditor.AssetDatabase.Contains(_compoundTerrainData))
            {
